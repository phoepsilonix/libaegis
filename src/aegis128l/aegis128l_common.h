...
static inline void
aegis128l_xor_keystream(uint8_t *const dst, const uint8_t *const src, aes_block_t *const state)
{
    ...
    aegis128l_update(state, zero, zero);
}
...
stream_xor(uint8_t *out, const uint8_t *in, size_t len, const uint8_t *npub, const uint8_t *k)
{
    ...
    for (i = AEGIS_STREAM_XOR_BULK(out, in, len, state); i + RATE <= len; i += RATE) {
        aegis128l_xor_keystream(out + i, in + i, state);
    }
    if (len % RATE) {
        memset(src, 0, RATE);
        memcpy(src, in + i, len % RATE);
        aegis128l_xor_keystream(dst, src, state);
        memcpy(out + i, dst, len % RATE);
    }
}
