...
int
aegis256x4_state_encrypt_update(aegis256x4_state *st_, uint8_t *c, const uint8_t *m, size_t mlen)
{
    return implementation->state_encrypt_update(st_, c, m, mlen);
}

int
aegis256x4_state_encrypt_final(aegis256x4_state *st_, uint8_t *mac, size_t maclen)
{
    if (maclen != 16 && maclen != 32) {
        errno = EINVAL;
        return -1;
...
int
aegis256x4_state_decrypt_update(aegis256x4_state *st_, uint8_t *m, size_t mlen_max, size_t *written,
...
int
aegis256x4_state_decrypt_final(aegis256x4_state *st_, uint8_t *m, size_t mlen_max, size_t *written,
